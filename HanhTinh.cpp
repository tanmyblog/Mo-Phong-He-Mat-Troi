...
float viewerPositionXY[2] = { 0.0, 0.0 };
float viewerPositionZ = 0.0;
...
void orbit()
{
	...
	for (i = 0; i<8; i++) {
		...
		//glTranslatef(viewerPositionXY[0], viewerPositionXY[1], viewerPositionZ);
		glBegin(GL_POINTS);
		double ang1 = 0.0;
		int i = 0;
		for (i = 0; i<800; i++)
...
void update(int value) {
	...
	{
		sx -= 0.00008; sy -= 0.00008; sz -= 0.00008;
	}
	else { sx += 0.00008; sy += 0.00008; sz += 0.00008; }
...
void mouseFunc(int button, int state, int x, int y)
{
	...
	{
	case GLUT_ACTIVE_SHIFT: shiftActive = 1;
		break;
	case GLUT_ACTIVE_ALT: altActive = 1;
		break;
	case GLUT_ACTIVE_CTRL: ctrlActive = 1;
		break;
...
void mouseMotionFunc(int x, int y)
{
	...
	else if (middleMouseButtonActive)
	{
		...
		if (!lastXOffset == 0.0)
		{
			viewerPositionXY[0] -= (xOffset - lastXOffset) / 8.0;
		...
		if (!lastYOffset == 0.0)
		{
			viewerPositionXY[1] += (yOffset - lastYOffset) / 8.0;
	...
	else if (rightMouseButtonActive)
	{
		...
		if (!lastZOffset == 0.0)
		{
			viewerPositionZ -= (zOffset - lastZOffset) / 5.0;
